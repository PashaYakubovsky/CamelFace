...
void main() {
    ...
    xl += fbm(vec2(uv.x * 2.0, uv.y * 0.5 * 2.0)) * 1.5;
    // ********* v v v opzionale 
    // xl += (fbm(vec2(uv.x * 2.0 + 234.23424, uv.y * 0.3 * 2.0)) * 2.0 - 1.0) * 0.35;
    // ********* ^ ^ ^ opzionale - END
    xl -= pow(fbm(vec2(uv.x * 8.0, uv.y * 2.2 * 2.0)), 2.5) * 0.45;
    
    // noise generale, per distorcere un po' le linee verticali 
    xl += fbm(vec2(uv.x * 2.0, uv.y * 6.0 * 2.0)) * (0.2 + sin(uv.y + time) * 0.1);
    ...
    float topOfTheLineFadingLength = 0.01;
    ...
    // gl_FragColor = vec4(vec3(1.0, 0.0, 0.0), a);
}
`
