...
class FresnelScene {
	...
	addDebug() {
		...
		// const meshPhysicalMaterialFolder = this.gui.addFolder(
	...
	animate(): void {
		...
		if (!this.renderer) {
			// animate fresnel
			if (this.material) {
				this.material.uniforms.uFolloff.value = Math.sin(this.time)
				this.material.uniforms.uFresnelPower.value = Math.cos(this.time)
			}
		}
