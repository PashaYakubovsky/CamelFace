...
class MophingScene {
		...
		this.gltfLoader.load('/AnimatedHuman.glb', (gltf: GLTF) => {
			...
			this.material = new THREE.ShaderMaterial({
				...
				uniforms: {
					...
					uStripeMultiplier: new THREE.Uniform(this.debugObject.stripeMultiplier),
					uFresnelPower: new THREE.Uniform(this.debugObject.fresnelPower),
					uHolographicMultiplier: new THREE.Uniform(this.debugObject.holographicMultiplier)
				}
			});
			...
			this.clip = THREE.AnimationClip.findByName(this.clips, 'Human Armature|Working');
	...
	addDebug() {
		...
		this.gui.addColor(this.debugObject, 'floorColor').onChange(() => {
			...
			// @ts-ignore
			this.floorPlane.material.color = new THREE.Color(this.debugObject.floorColor);
		});
		...
		hologramFolder
			...
			.onChange(() => {
				if (this.material) {
					this.material.uniforms.uStripeMultiplier.value = this.debugObject.stripeMultiplier;
				}
			});
		hologramFolder
	...
	animate(): void {
		...
		this.camera.rotation.y = THREE.MathUtils.lerp(this.camera.rotation.y, this.mouse.x, 0.05);
