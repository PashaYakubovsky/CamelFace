...
class ParticlesScene {
	...
	simplexNoiseVert(ver: Float32Array): Float32Array {
		...
		for (let i = 0; i < count; i++) {
			...
			const y =
				getNoise(x * noiseScale, z * noiseScale, this.time) * noiseRoughness
	...
	addDebug() {
		...
		this.gui
			...
			.onFinishChange(() => {
				const aspectRatio =
					this.params.particlesWidth / this.params.particlesHeight
		...
		folderHoverNoise.add(this.params, "hoverNoise").onFinishChange(() => {
			if (this.material)
				this.material.uniforms.uHoverNoiseEnabled.value = this.params.hoverNoise
		})
		...
		folderHoverNoise
			...
			.onFinishChange(() => {
				if (this.material)
					this.material.uniforms.uRadius.value = this.params.hoverRadius
			})
		...
		folderHoverNoise
			...
			.onFinishChange(() => {
				if (this.material)
					this.material.uniforms.uScale.value = this.params.hoverScale
			})
		...
		folderHoverNoise
			...
			.onFinishChange(() => {
				if (this.material)
					this.material.uniforms.uStrength.value = this.params.hoverStrength
			})

		const folderShaderNoise = this.gui.addFolder("Shader Noise")
		folderShaderNoise.add(this.params, "shaderNoise").onFinishChange(() => {
			if (this.material)
				this.material.uniforms.uEnabledNoise.value = this.params.shaderNoise
		})
		folderShaderNoise
			...
			.onFinishChange(() => {
				if (this.material)
					this.material.uniforms.uNoiseRoughness.value =
						this.params.shaderNoiseRoughness
			})
		folderShaderNoise
	...
	animate(): void {
		...
		const elapsedTime = this.clock.getElapsedTime() * 0.1
