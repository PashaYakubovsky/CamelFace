...
void main() {
    ...
    float noise = 0.5 + snoise(
        vec3(vPosition.x, 0.0, vPosition.z) * 40.0  + iTime * 0.1
    );
    noise = mix(noise, 1.0, grad);

    float fadeToTop = 1.5 - smoothstep(1.0, 0., grad);

    // float fog = smoothstep(-1., 7.0, length(vViewPosition*vec3(2.0, 1.0, 1.0)));
